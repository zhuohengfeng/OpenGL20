...
public class Triangle {
    ...
    int muMVPMatrixHandle;//总变换矩阵引用(在顶点着色器中)
    int maPositionHandle; //顶点位置属性引用(在顶点着色器中)
    int maColorHandle; //顶点颜色属性引用(在顶点着色器中)
    ...
    public void initVertexData()//初始化顶点数据的方法
    {
        //顶点坐标数据的初始化
        vCount=3;  // 一共有3个顶点
        final float UNIT_SIZE=0.2f; // 坐标系是 原点是(0,0)在中心， 然后-1 ~ 1
        float vertices[]=new float[]//顶点坐标数组
                {
                        -4*UNIT_SIZE,0,0,  // 逆时针的3个点
                        0,-4*UNIT_SIZE,0,
                        4*UNIT_SIZE,0,0,
                };

        ByteBuffer vbb = ByteBuffer.allocateDirect(vertices.length*4);  // 有几个顶点，每个顶点3个坐标，然后每个坐标为float类型，占用4个字节
    ...
    public void initShader(MyGLSurfaceView mv)
    {
        ...
        //获取程序中顶点位置属性引用 Attrib
        maPositionHandle = GLES20.glGetAttribLocation(mProgram, "aPosition");
        //获取程序中顶点颜色属性引用 Attrib
        maColorHandle= GLES20.glGetAttribLocation(mProgram, "aColor");
        //获取程序中总变换矩阵引用 Uniform
    ...
    public void drawSelf()
    {
        ...
        //将顶点颜色数据传送进渲染管线
        GLES20.glVertexAttribPointer(
                maColorHandle, // 顶点颜色引用
                4, // 每个顶点有4个值， R, G, B, A
                GLES20.GL_FLOAT,
                false,
                4*4, // 每个值占4个字节,  stride指定从一个属性到下一个属性的字节跨度，允许将顶点和属性打包到单个数组中或存储在单独的数组中
                mColorBuffer
        );
        GLES20.glEnableVertexAttribArray(maPositionHandle);//启用顶点位置数据
        GLES20.glEnableVertexAttribArray(maColorHandle);//启用顶点着色数据
        //绘制三角形
        GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 0, vCount); // 有3个顶点，就是一个普通的三角形
    }
